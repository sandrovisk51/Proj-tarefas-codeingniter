    ...
    <div class="container">
        ...
        <div class="row justify-content-md-center">
            <div class="col-6 border border-primary rounded px-5 py-5">
                <h5>Cadastrar Nova Tarefa</h5>
                <?php if(isset($validacao)):?>
                    <div class="alert alert-danger"><?= $validacao->listErrors() ?></div>
                <?php endif;?>
                <form action="/principal/salvar" method="post">
                    <div class="mb-2 mt-3">
                        <label for="InputForDescricao" class="form-label">Descrição da Tarefa</label>
                        <input type="text" name="descricao" class="form-control" id="InputForDescricao" value="<?= set_value('descricao') ?>">
                    </div>
                    <div class="form-group mb-2">
                        ...
                        <select class="form-control" name="responsavel" id="InputForResponsavel">
                            ...
                            <?php
                            var_dump($listaResponsaveis);
                                if($listaResponsaveis):
                                    foreach($listaResponsaveis as $responsavel):
                                     echo '<option value="'.$responsavel["id"].'">'.$responsavel["res_descricao"].'</option>';
                                    endforeach;
                                endif;
                           ?>
                        </select>
                            ...
                            <?php
                                if($listaCategorias):
                                    foreach($listaCategorias as $categoria):
                                     echo '<option value="'.$categoria["id"].'">'.$categoria["cat_descricao"].'</option>';
                                    endforeach;
                                endif;
                           ?>
                      </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="dataFinalizacao" class="form-control" id="InputForDataFinalizacao">
                    </div>
